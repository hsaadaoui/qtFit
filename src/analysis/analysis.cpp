...
{
    anaGUI->setupUi(this);
    libraryPath = "/usr/local/qtfit/fct/";
    tabWidget_initials();
    dataOptions();
    comboBox_fct();
    comboBox_fitAs();
...
//Create combo of data type
void Analysis::dataOptions()
{
    QStringList Type;
    anaGUI->comboBox_dataType->clear();
    Type<<"ASCII";
    anaGUI->comboBox_dataType->addItems(Type);

    QStringList Format;
    anaGUI->comboBox_dataFormat->clear();
    Format<<"x y" <<"x y dy"<<"x dx y dy"<<"x dx y dy1 dy2"
         <<"x dx1 dx2 y dy"<<"x dx1 dx2 y dy1 dy2";
    anaGUI->comboBox_dataFormat->addItems(Format);
    anaGUI->comboBox_dataFormat->setCurrentIndex(1);
}
...
bool Analysis::fileList()
{
    ...
    dataFormat.clear();
    subType.clear();
    subType << anaGUI->comboBox_dataFormat->currentText();
    QString dataType = anaGUI->comboBox_dataType->currentText();
    int  formatIndex = anaGUI->comboBox_dataFormat->currentIndex();
    ...
    if ( dataType == "ASCII" )
    {
        ...
        {
            QMessageBox::warning(this,tr("Error"),
                                 tr("None of these special characters"
                                    "are allowed \":;|<>?*&^%$#@!()\".\n"
...
//Graphics
void Analysis::plots(bool fit)
{
    //needed for Graphics the graphs
    QVector<QString> fileNames,xLabels,yLabels;
    QVector<int>     xformat,yformat;
    QString          basename,rawData,fitData;
    //specifies the arguments of graphs
    ...
    //send signals to plotting
    Graphics::instance()->qcp(fileNames,xLabels,yLabels,xformat,yformat);
    if (!fit)
    {
